...
| 补第三十处 —— 缺陷 28 的另一半 | 修订说明行；它提到的那个 job 已端到端跑绿（run `36233724927`）并进 `required-checks`，原先登记的 OPEN-01 已按判据删除 |
...
| 第 34 步的 CI 部分当时记为「未端到端验证」 | 引用的是一句**被推翻的旧判断**（§7 第 34 步的说明 2026-09-26 已改），用来说明判断怎么变的；它指的那条欠账（§8 与 OPEN-01）已于同日跑绿、条目按判据删除 |
