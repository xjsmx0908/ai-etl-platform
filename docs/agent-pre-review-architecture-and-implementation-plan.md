...
| `assess_knowledge_fitness` | 记录是否适合当前知识空间、能否作为正式知识；可选类型备注不是发布开关 |

`get_review_context` 读取空间用途。空间没有用途时，不启用“适不适合”门禁；“能不能当知识用”仍可按模型判断拦截。版本差异、政策库和跨文档冲突不在本切片。
...
- 记录延迟、token 和人工转交结果；
- 证据由 `scripts/release-center-real-model-scenarios-acceptance.sh` 写入
  `artifacts/release-center-real-model-scenarios-acceptance/`。

完成条件：功能矩阵、Go/Python/Web 测试和真实模型验收全部通过。

## 10.1 剩余工作顺序

按整体功能完整度，实施顺序为：累计 token 预算控制（代码和真实模型专项验收已完成） → 预审恢复与幂等专项验收（代码、单测与隔离栈 Redis/Compose 故障证据已完成） →
真实模型四场景验收（已完成） → 通知、Saga 与外部工作流（R5，已完成） → Review Report 生命周期（R3，已完成）
→ P2.4-R6 知识空间适配预审（已完成） → P2.4-R6-ACCEPT 真实环境验收（已完成） → 完整语义/隐私/合规审查。前 7 项已关闭；版本对比/政策库/冲突检查仍暂缓。

## 11. MVP 完成定义

当前状态（2026-09-08）：步骤 1～4 已完成。确定性单元/契约测试、Go 全模块、Web
lint/build、发布中心隔离栈业务矩阵、真实模型预算终止、Review Agent 部署恢复和真实模型
场景验收已通过；R1 MVP 标记为 completed。R6 用途匹配、空间不适配和不能作为正式知识
的隔离栈与真实模型验收已通过。

满足以下条件即可结束首版，不要求同时建设未来平台能力：

- Agent 根据 observation 决定下一工具或结束，存在真实的多步反馈循环；
- 只注册受控只读工具，且 tenant/candidate 由服务端强绑定；
- findings 全部引用当前 Run 的有效 chunk 或 Step；
- 步骤、时间和累计 token 预算可强制终止运行；
...
- P2.4-R6 知识空间适配预审（已完成）：空间用途、适不适合、能否作为正式知识；类型备注不作为发布开关；聊天/草稿由服务端下限拦截；真实环境验收已通过；
- 版本差异审查和跨文档冲突检查（暂缓）；
- 企业政策库和外部法规检索（暂缓）；
- 独立 Evidence Ledger、claim 图和证据生命周期；
- shadow/canary 自动化与高级模型评测平台；
- 完整语义/隐私/合规审查。不得把材料类型分类器做成发布门禁。

## 13. 结论

最小方案只改造预审决策方式，不重建 Agent 基础设施：复用现有 Orchestrator，让模型
在受控只读工具之间完成“计划—工具—观察—再计划/报告”的闭环，再由现有服务端门禁
