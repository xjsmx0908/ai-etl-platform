...
| 已知未修 | **跨层索引一致性的剩余项**（§1.3.1）：对账脚本 `scripts/check-index-consistency.py`（`499ed26` `aae0d59` `6930663` `67d79ab` `4c995c4`）已把四层摆在一起比。其中「引用无法在页面上核对」**17 份已修掉 12 份**（缺陷 23 / 24 / 25，`99c119a` `4133512` `0e12588`），**剩余 5 份已逐条归因为「按设计隐藏」**；登记表块数那 **99 份已回填**（`5989882`，99 → 0）；**两条判定此前误报，已按身份收窄**（`4c995c4`：`duplicate_chunk_points` 48 → 0、`space_key_unset` 45 → 8，剩余 8 条已归因为「只有不带身份的旧拷贝」）；真正仍挂着的只有 **2 份**已发布文档不在全文索引里（历史数据回填，待决策） |
...
### 1.3.1 跨层索引一致性：对账已建并扩到检索过滤字段与登记表计数（`499ed26` `aae0d59` `6930663` `67d79ab` `4c995c4`），`citation_unverifiable` 17 → 5（12 份已修为缺陷 23 / 24 / 25，剩余 5 份已逐条归因为「按设计隐藏」），其余待决策
...
（上表是报告 `artifacts/index-consistency/index-consistency-20260924T080631Z.json` 的读数，即回填、修好分页、并把两条判定按身份收窄之后的线上状态；`permission_drift` 特意留着，因为删掉它会让读者以为没检查过。前几版报告里的 `count_mismatch` 1、`registry_count_stale` 99、`space_key_unset` 45、`duplicate_chunk_points` 48 四处变化的原因都在下面几段。）
...
- **跨层对账的 `citation_unverifiable` 还剩 5 条、`space_key_unset` 还剩 8 条「已归因的先决条件」，而脚本本身不区分它们与真风险**。脚本比的是「端点 vs 存储」—— 一份文档有健康的已发布身份时，不带代际身份的拷贝按设计就该被两端同时滤掉，所以这 13 条永远会出现在报告里（`space_key_unset` 那 8 条更直接：这些 chunk id 在存储里**只有**不带身份的旧拷贝）。后果是**这个脚本现在永远 `exit 1`**，接进 CI 之前必须先定一件事：要么把「已归因的先决条件」拆成不影响退出码的一类，要么把判据改成「先按发布策略过滤存储侧再比」（后者等于在 Python 里复刻 Go 的策略，得配一条防漂移的契约测试）。两条都要先定，且与上面那条「怎么接进 CI」是同一个决策 —— 所以没有单方面改退出码语义。
