...
| 已修复项 | 20 个功能缺陷 + ES 永久 yellow（真因是**单节点配了 1 副本**，不是磁盘水位）+ P0 备份与恢复演练（§1.2）+ ES 水位百分比化、宿主机磁盘告警、磁盘回收（§1.1.2）+ 配置键一致性：3 个切块死键、26 个未文档化的键、3 个 compose 漏传的检索键、5 个「模板写了但两边都没接」的接线缺口（§2.2 / §2.3 / §8）。见 §1.1、§1.2、§1.3、§2.2、§2.3、§8 |
...
### 1.3 已修复的 20 个缺陷（前五处同一形状：失败被固化，重试变成复读）
...
第 26 步是 §6 第三条「诚实标注」的收口，排在最后是因为它**必须有源码改动**（要往运行中的
栈里注入一次真实中断），与第 25 步互斥。它的结论是分两半的：恢复语义本身没问题 ——
...
- **代码改动限于四处**：§1.3 列出的 20 个缺陷，§1.1.2 的 ES 水位百分比化 + 宿主机磁盘告警
