...
| 已修复项 | 25 个功能缺陷（第 23–25 个：跨层口径不一致导致引用不可核对，同一判据的三种落空，§1.3.1，`99c119a` `4133512` `0e12588`）+ ES 永久 yellow（真因是**单节点配了 1 副本**，不是磁盘水位）+ P0 备份与恢复演练（§1.2）+ ES 水位百分比化、宿主机磁盘告警、磁盘回收（§1.1.2）+ 配置键一致性：3 个切块死键、26 个未文档化的键、3 个 compose 漏传的检索键、5 个「模板写了但两边都没接」的接线缺口（§2.2 / §2.3 / §8）。见 §1.1、§1.2、§1.3、§2.2、§2.3、§8 |
| 已知未修 | **跨层索引一致性的剩余项**（§1.3.1）：对账脚本 `scripts/check-index-consistency.py`（`499ed26` `aae0d59` `6930663`）已把四层摆在一起比。其中「引用无法在页面上核对」**17 份已修掉 12 份**（缺陷 23 / 24 / 25，`99c119a` `4133512` `0e12588`），**剩余 5 份已逐条归因为「按设计隐藏」**；仍挂着的还有 **2 份**已发布文档不在全文索引里、**1 份** ES/Qdrant 块数不一致、**99 份**登记表自己的块数没写（页面恒显示「—」，是缺陷 3 的历史遗留）、**45 份**存储点缺空间过滤键、**48 份**同一 chunk id 有重复存储点（后五项都是历史数据回填，待决策） |
...
### 1.3 已修复的 25 个缺陷（前五处同一形状：失败被固化，重试变成复读；第 21–22 处是切块边界；第 23–25 处是跨层口径不一致的三种形状：端点隐藏了存储仍在提供的块、端点按**正文**隐藏了同一文档里正文相同的另一个 chunk id、端点在发布策略**之前**就合并了同一 chunk id 的重复存储点）
...
### 1.3.1 跨层索引一致性：对账已建并扩到检索过滤字段与登记表计数（`499ed26` `aae0d59` `6930663`），`citation_unverifiable` 17 → 5（12 份已修为缺陷 23 / 24 / 25，剩余 5 份已逐条归因为「按设计隐藏」），其余待决策
...
| `keyword_unsearchable` | **2** | 已发布文档在 ES 里一个块都没有 —— 全文召回静默丢掉这份文档 | **未修**：需回填历史数据（待决策） |
| `count_mismatch` | 1 | ES 与 Qdrant 的块数不一致 | **未修**：同一批历史数据 |
| `registry_count_stale` | **99** | 登记表自己的块数（`chunks_done` / `chunks_total`）与存储不一致 —— 页面渲染的就是这两个数 | **写入侧已好、历史行未回填**：全部是缺陷 3 的历史遗留（见下），属数据修复（待决策），**不登记为新缺陷** |
| `space_key_unset` | **45** | 存储点的 `metadata.knowledge_base_id` 不是该文档的知识空间 —— 向量分支在该空间内召回不到它们，只剩关键词可召回 | **未修**：同一批历史数据。ES 侧同一个键是有的，所以端到端指标看不出来 |
| `duplicate_chunk_points` | **48** | 同一 chunk id 有多份存储点 —— 重复入库是**追加**而不是替换 | **未修**：同一批历史数据。实测其中**没有**同一身份的两份（0 个 chunk id），所以不影响评审绑定的重复守卫 |
| `permission_drift` | 0 | 存储点的 `permission` 与登记值不一致 | 无 |

（上表是同一份报告 `artifacts/index-consistency/index-consistency-20260924T072325Z.json` 的读数；`permission_drift` 特意留着，因为删掉它会让读者以为没检查过。）
...
**剩余 5 份 `citation_unverifiable` 已逐条归因，不是未修的缺陷**：这 5 份是 `SEC-2024-001`、`PM-2024-001`、`PROC-2024-001`、`FIN-2024-001`（各 1 个被隐藏的 chunk id）与 `doc-1788331217639048684`（98 个；该文档存储里共 166 个不同的 chunk id，页面显示 68 个）。逐个打印存储点后原因只有一种：**被隐藏的 chunk id 在存储里的每一份拷贝都不带代际身份**（`document_version_id` 与 `generation_id` 都是空串），而这 5 份文档都有健康的已发布身份 —— 于是详情页策略与检索策略**同时**把它们滤掉（两边都按 identity 精确匹配），引用不可能指向它们。这不是推的：对这 5 份各发一个用隐藏块原文构造的真实问题，检索只返回了已发布的那一块（4 份 demo 文档返回 `_0000`，`doc-1788331217639048684` 返回该文档 0 块）。所以这 5 条是**先决条件**（脚本比的是「端点 vs 存储」，它不问「检索是否会返回」），不是线上风险。把它们与真风险分开报、以及分开后退出码怎么定，与「对账怎么接进 CI」是同一个待决策项（§8）。

**`keyword_unsearchable` 的成因与处置（已定位到行，未修）**：`doc-1788316441601845458`（kafka-recovery.txt）与 `doc-1788317207289560094`（吉项目与服务器运维手册.docx），两份都 `published` + `active` + `completed`，但 ES 里一个块都没有。**判别实验证明写入路径现在是好的**：新上传一份 `.txt` → `endpoint=2 / qdrant=2 / ES=2`。所以这两份是历史遗留（2026-09-02 入库，早于当前的索引/清单机制），**但系统没有任何检查能发现它**。修法是数据回填（按 Qdrant 重放 ES + 补写合成 manifest 行），属数据迁移，**待决策**。

**`registry_count_stale` 是缺陷 3 的历史遗留，不是新缺陷（已判定，未回填）**：`documents.chunks_done / chunks_total` 是块数的**去规范化缓存**，前端两处直接渲染它 —— `web/app/(app)/documents/page.tsx:357` 与 `web/app/(app)/documents/[id]/page.tsx:274`，表达式都是 `doc.chunks_total ? \`${doc.chunks_done ?? 0}/${doc.chunks_total}\` : "—"`。实测 114 份 `completed` 文档里 **99 份**与存储不一致，且**全部**是 `chunks_total = 0`：其中 72 份 `chunks_done` 也是 0，另 27 份 `chunks_done` 已经是对的、只有 total 没写。按 `created_at` 分组看得很干净 —— **≤ 2026-09-10 的行全部是 0（99 份），≥ 2026-09-22 的行全部已写入且正确（15 份）**，分界线正是缺陷 3 的修复 `4cf5963`（2026-09-21）。所以写入侧现在是好的，**这是 99 行历史数据从未回填**，不是活缺陷；而且它们**全部**落在 `chunks_total = 0`，也就是页面恒显示「—」，读者看不到任何数字。修法是数据回填（把 total 补成该文档已发布代际的块数），属数据修复，**待决策**，因此**不登记为第 26 个代码缺陷**。判定基线是「去重后的存储块数」而非「已发布代际的块数」—— 这一层看不到计数是给哪一次入库写的（计数按次写入，重复入库的文档在存储里留着每个代际），所以 `detail` 必须分开写：total 为 0 才是「页面显示 —」，total 非 0 时页面显示的是一个对不上的数字；`findings` 同时给出 `stored_chunks` 与 `stored_points`，让「计数过期」与「存储留着被取代的拷贝」可区分。
...
第 32 步（已完成） §1.3.1 对账第五个判定 `registry_count_stale`：登记表自己的块数（`chunks_done`/`chunks_total`）与存储不一致时也报出来，并按「页面显示 —」与「页面显示一个对不上的数字」两种读数分开写结论（`6930663`）—— 线上 99 份，归因为缺陷 3 的历史遗留（全部创建于 ≤ 2026-09-10，即 `4cf5963` 之前），写入侧已好、属待决策的数据回填，**不登记为新缺陷**
```
...
- **代码改动集中在这些章节**：§1.3 列出的 25 个缺陷，§1.1.2 的 ES 水位百分比化 + 宿主机磁盘告警
...
- **跨层对账的 `citation_unverifiable` 还剩 5 条「已归因的先决条件」，而脚本本身不区分它们与真风险**。脚本比的是「端点 vs 存储」—— 一份文档有健康的已发布身份时，不带代际身份的拷贝按设计就该被两端同时滤掉，所以这 5 条永远会出现在报告里。后果是**这个脚本现在永远 `exit 1`**，接进 CI 之前必须先定一件事：要么把「已归因的先决条件」拆成不影响退出码的一类，要么把判据改成「先按发布策略过滤存储侧再比」（后者等于在 Python 里复刻 Go 的策略，得配一条防漂移的契约测试）。两条都要先定，且与上面那条「怎么接进 CI」是同一个决策 —— 所以没有单方面改退出码语义。
- **登记表自己的块数（`documents.chunks_done` / `chunks_total`）有 99 行是历史遗留，没有回填，也还没决定要不要回填**。写入路径已经是对的（缺陷 3，`4cf5963`，2026-09-21）：实测 114 份 `completed` 文档里，**≤ 2026-09-10 创建的 99 行 `chunks_total` 全是 0，≥ 2026-09-22 创建的 15 行全部已写入且与存储一致**。所以这不是活缺陷，是 99 行数据没补。它们**全部**落在 `chunks_total = 0`，也就是文档清单与详情页恒显示「—」—— 读者看到的是「没有数字」，不是「错的数字」，这也是为什么它一直没有被发现。回填要回答一个产品问题：这两个数该按**哪一代**算（当前是「最后一次入库」，而重复入库的文档在存储里留着每个代际，已发布代际可能不是最后一次）。定完才能写回填脚本，所以**没有先改**。对账脚本已把这个读数报出来（`registry_count_stale`），不影响退出码语义的讨论（它和别的判定一样是「有问题就报」）。
