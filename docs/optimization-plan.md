...
> **修订说明（2026-09-22；2026-09-23 补第十七至二十处，同日完成全文一致性审计并修掉 7 处漂移，见 §8 末条；2026-09-24 补第二十一、二十二处切块边界缺陷；同日加跨层索引一致性对账，查出 17 份文档的引用无法在页面上核对、2 份已发布文档不在全文索引里；同日把「引用无法核对」修掉（第二十三处，端点不再隐藏存储仍在提供的块），线上复跑 19/19 引用可核对）**：初稿结论是「风险不在功能，在运维底座」。随后在部署环境上做了逐条复现的
> 缺陷排查，找到并修复了 23 个**功能/可靠性**缺陷（见 §1.3）：5 个属于「失败被固化、重试变成复读」，
...
| 已修复项 | 23 个功能缺陷（第 23 个：跨层口径不一致导致引用不可核对，§1.3.1，`99c119a`）+ ES 永久 yellow（真因是**单节点配了 1 副本**，不是磁盘水位）+ P0 备份与恢复演练（§1.2）+ ES 水位百分比化、宿主机磁盘告警、磁盘回收（§1.1.2）+ 配置键一致性：3 个切块死键、26 个未文档化的键、3 个 compose 漏传的检索键、5 个「模板写了但两边都没接」的接线缺口（§2.2 / §2.3 / §8）。见 §1.1、§1.2、§1.3、§2.2、§2.3、§8 |
| 已知未修 | **跨层索引一致性的剩余两项**（§1.3.1）：对账脚本 `scripts/check-index-consistency.py`（`499ed26`）已把四层摆在一起比。其中「引用无法在页面上核对」**已修**（缺陷 23，`99c119a`，线上 19/19 引用可核对）；仍挂着的只剩 **2 份**已发布文档不在全文索引里（历史数据回填，待决策）与 **1 份** ES/Qdrant 块数不一致 |
...
### 1.3 已修复的 23 个缺陷（前五处同一形状：失败被固化，重试变成复读；第 21–22 处是切块边界；第 23 处是跨层口径不一致：端点隐藏了存储仍在提供的块）
...
### 1.3.1 跨层索引一致性：对账已建（`499ed26`），`citation_unverifiable` 已修为缺陷 23（`99c119a`），其余待决策

上面 22 条都是「某一层自己算错了」。第 23 条不同：**四层各自看都对，合起来不一致，而没有任何东西比较它们**。
...
| 判定 | 数量 | 含义 | 状态 |
| --- | --- | --- | --- |
| `citation_unverifiable` | **17 → 0** | 端点隐藏了 Qdrant 仍在提供的块 —— 引用里的 chunk id 在文档页面上找不到 | **已修**（缺陷 23，`99c119a`）：端点不再隐藏被包含的块，线上复跑 19/19 引用可核对 |
...
- **代码改动集中在这些章节**：§1.3 列出的 23 个缺陷，§1.1.2 的 ES 水位百分比化 + 宿主机磁盘告警
