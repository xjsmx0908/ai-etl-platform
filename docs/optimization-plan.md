...
- **ES 死信队列（`es:index:deadletter`）仍然只写不读 —— 这一轮只补偿了后果，没有修那条链路**。
  ...
  没有告警规则读它，也没有清理或重放入口。这一轮做的是把**已经丢掉的那 2 份**回填回去、并让对账能自动
  发现同类问题；**没有做的是让死信本身传得到会触发动作的地方**（例如计入一个 Prometheus 指标并配一条
  告警，或者提供一个带人工确认的重放入口）。没顺手做的原因是它要动告警栈与运行中的服务，而这一轮的
  目标是把已知的两件事收口。**下次磁盘再越过 flood-stage 水位，同样的事会再发生一次，区别只是对账会在
  它发生后的第一次运行时把它报出来。** 顺带记一笔环境事实：写这份记录时宿主机磁盘仍是 **90%**
  （170G/197G，可用 19G），而 flood-stage 默认是 95% —— 触发条件并没有消失。
