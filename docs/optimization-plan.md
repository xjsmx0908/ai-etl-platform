...
| 最紧急项 | **没有 P0 挂着**。三项 P0 都已处理：ES 永久 yellow（§1.1.1）、全栈无备份（§1.2）、磁盘濒满（§1.1.2）。**但磁盘这条要说清楚**：2026-09-23 那次清到 63GB 可用，四天内涨回 95%；2026-09-26 复测是 `197G / 179G 已用 / 10G 可用 / 95%`，回收到 92%（16GB 可用），而**一次 `docker compose up -d --build` 就吃掉约 5GB**（重建后复测 11GB 可用 / 95%）。所以它不是「已解决」，而是「已有一个默认 dry-run、带范围守卫、可重复执行的回收脚本」（`scripts/reclaim-host-disk.sh`，`b131844`），且**本项目自己的可回收残留只剩约 1GB** —— 剩下约 34GB 未引用镜像属别的项目或共享构建基镜像，见 §8。下一步按 §7 的执行顺序走 |
| 已修复项 | 30 个功能缺陷（第 23–25 个：跨层口径不一致导致引用不可核对，同一判据的三种落空，§1.3.1，`99c119a` `4133512` `0e12588`；第 26 个：弃答把四种原因写成同一句话，而那句话在「文档存在但未发布 / 不在访问范围」时与事实相反，§1.3，`a64da10`；第 27 个：查询结果与弃答从不进指标，4 条已注册的 query 指标从没被喂过数据，「系统不再作答」在生产里没有任何信号，§1.3，`128100a`；第 28 个：ES 死信列表没有上限，而它与重试队列、检查点、租约、outbox 共用 512MB `noeviction` 的 Redis —— 积压到上限会先填满实例、再让所有写入一起失败，而积压它的场景与产生死信的场景是同一个，§1.3 / §8，`17d436f`；第 29 个：问句指代上文时系统自己挑一个指代对象作答 —— 引文忠实、grounding 通过，错的是前提，而单轮接口里没有任何东西能决定那个指代，§1.3 / ADR 0012，`bb7b270` `c77e55d` `d15ac00`；第 30 个：ES 死信列表**没有消费者** —— 只有写没有读，条目唯一的出口是到达上限时被裁掉，「已解决的故障」永远挂在面板上，而上限在拿旧故障的证据换新故障的证据，§1.3 / §8，`2a2fba9`）+ ES 永久 yellow（真因是**单节点配了 1 副本**，不是磁盘水位）+ P0 备份与恢复演练（§1.2）+ ES 水位百分比化、宿主机磁盘告警、磁盘回收（§1.1.2）+ 配置键一致性：3 个切块死键、26 个未文档化的键、3 个 compose 漏传的检索键、5 个「模板写了但两边都没接」的接线缺口（§2.2 / §2.3 / §8）。见 §1.1、§1.2、§1.3、§2.2、§2.3、§8 |
| 已知未修 | **跨层索引一致性的剩余项**（§1.3.1）：对账脚本 `scripts/check-index-consistency.py`（`499ed26` `aae0d59` `6930663` `67d79ab` `4c995c4` `5989882` `60756c1`）已把四层摆在一起比。其中「引用无法在页面上核对」**17 份已修掉 12 份**（缺陷 23 / 24 / 25，`99c119a` `4133512` `0e12588`），**剩余 5 份已逐条归因为「按设计隐藏」**；登记表块数那 **99 份已回填**（`5989882`，99 → 0）；**两条判定此前误报，已按身份收窄**（`4c995c4`：`duplicate_chunk_points` 48 → 0、`space_key_unset` 45 → 8，剩余 8 条已归因为「只有不带身份的旧拷贝」）；**「已发布文档不在全文索引里」那 2 份已回填**（`60756c1`，2 → 0，真因是一条只写不读的死信队列）。**对账的五个判定现在全部为 0 或已归因，剩余 13 条先决条件进基线（`expires` 2026-12-31），脚本已接 CI**。真正仍挂着的**不再是**「死信列表没有消费者」—— 2026-09-26 补上了消费者（缺陷 30，`2a2fba9`）：按「意图是否已满足」判定后 ack、判不了的一律留下，线上 90 → 0，见 §8；**重放**仍不做 —— 而本文上一版写的「无重放入口」是**高估**：`reconciler.replayFailed` 就是入口，只是那 9 条 `repair_attempts` 已达上限的 `failed` 行永久不可认领。仍挂着的是**宿主机磁盘**：实测已不是 90% 而是 **95%**（flood-stage 实测 97%、不是默认的 95%），2026-09-26 回收到 92%，但**一次重建就吃回约 5GB**；本项目自己的可回收残留只剩约 1GB，其余约 34GB 未引用镜像属别的项目或共享构建基镜像 —— 动它们要用户点头，见 §8 |
...
### 1.3 已修复的 30 个缺陷（前五处同一形状：失败被固化，重试变成复读；第 21–22 处是切块边界；第 23–25 处是跨层口径不一致的三种形状：端点隐藏了存储仍在提供的块、端点按**正文**隐藏了同一文档里正文相同的另一个 chunk id、端点在发布策略**之前**就合并了同一 chunk id 的重复存储点；第 26 处是弃答：四种原因共用同一句话，而那句话在「文档存在但未发布 / 不在访问范围」时与事实相反；第 27 处是生产侧可观测性：查询结果与弃答从不进指标，而 4 条已注册的 query 指标从没被喂过数据；第 28 处是容量：ES 死信列表没有上限，而它与重试队列、检查点、租约、outbox 共用同一个 512MB `noeviction` 实例 —— 积压先填满实例，再让所有写入一起失败；第 29 处是弃答的**前提**：问句用了指代上文的说法（「刚才那份」「上面说的」）时，系统自己挑一份检索结果当作指代对象作答 —— 引文忠实、grounding 通过，错的是**前提**，而单轮接口的请求里没有任何东西能决定那个指代，所以既有校验一条都不响；第 30 处是缺陷 28 的另一半：死信列表**只有写、没有读**，条目唯一的出口是到达上限时被裁掉，于是「已解决的故障」永远挂在面板上、深度告警永远非零，而上限是在拿旧故障的证据换新故障的证据 —— 消费者按「意图是否已满足」判定后 ack，判不了的一律留下）
...
| 29 | 问答对**指代上文的问句**自己挑一个指代对象作答：引文忠实、grounding 通过，错的是**前提** —— 单轮接口的请求里没有任何东西能决定「刚才那份」指哪一份，于是模型拿当轮检索到的某一份顶上，系统以 `refusal_reason=none` 把一次猜测当成一次作答 | 修复前线上实测四句（`demo` 租户普通用户身份）：问「刚才那份文档里还写了什么？」→ 答「最后一份文档（员工手册）还包含以下内容…」、`refusal_reason=none`、来源 `demo-doc-handbook`；问「上面说的第二点是什么意思？」→ 答「『上面说的第二点』应指第一条文档中的『机密数据须经数据所有者授权，访问行为全程留痕』」、来源 `demo-doc-security`。两句的引文都能在来源里逐字找到、grounding 通过，所以既有校验一条都不响。对照两句不指代的问法：「它的上限是多少？」模型自己列出候选并反问（住宿/事假/年假）、「那这个呢？」直接弃答 —— 说明模型并非总在猜，缺陷是**没有判据**而不是模型不配合 | 两处：① `POST /v1/query` 的请求体只有 `question`，全仓没有 `conversation_id` / 会话历史，所以「指哪一份」这个信息在协议层就不存在，而检索总会返回若干份候选 —— 「挑一份」是必然发生的默认行为，不是偶发幻觉；② 没有任何代码检查问句里是否含**指代上文的说法**：`internal/query/service.go` 的五条弃答分支全部挂在「检索结果不够」上，而这里检索结果很够。修法不是让模型更小心（它已经会列候选并反问），是**在访问校验之后、检索之前**加一道判据：新增 `internal/retrieval/reference_policy.go`，`HasUnresolvedReference` 用 **47 条整短语**词表（「刚才那份」「上面说的」「之前提到的」「上一轮」等）判定，命中即返回第 6 个弃答原因 `context_required`，不检索、不生成、`sources` 三种全空。词表刻意用整短语而非「指示词 + 指代名词」运行时拼装，因为**拒掉一个能答的问题比它要替代的猜测更糟** —— 逐条排除项写进注释与 ADR 0012（裸「刚才」/「之前」、「上述内容」、「合同第三条」都不判；`RetrievalOnly` 诊断调用豁免，否则容量/诊断探针会被自己的探针句拦下）；已知边界：裸序数指代（「第二点是什么意思？」）不判，因为无法与「合同第三条规定了什么？」区分。弃答句三处同源必须同步（后端 `refusalAnswers`、前端 `REFUSAL_EXPLANATIONS`、评测轨 `NEGATIVE_FALLBACK_MARKERS` —— 漏补会把**正确的弃答记成幻觉作答**），并用跨语言契约测试把「每条弃答句评测轨都认得」钉住 | `bb7b270` `c77e55d` `d15ac00` |
| 30 | ES 死信列表**没有消费者**：它只有写、没有任何读，条目唯一的出口是到达上限时被裁掉 —— 于是「已解决的故障」永远挂在面板上、深度告警永远非零，而上限是在**拿旧故障的证据换新故障的证据**。三周前那 90 条死信静默躺着，正是这一点让「差异被记录下来」不等于「有人处理」 | 线上实测：那 90 条死信（2 份文档、32 个不同 chunk id、全部 `retry=12`、同一条 `429 cluster_block_exception`）的 32 个 chunk id 在 ES 里**命中 32/32** —— 即 2026-09-24 回填之后，这 90 条的**意图早已全部满足**，它们已是纯过期的堆积，唯一的害处是占 2.1MB 并让深度告警一直非零。部署后重启 worker：启动日志 `es dead-letter drain pass complete scanned=90 drained=90 retained_missing=0 retained_unavailable=0 retained_unsupported=0`；`LLEN` 90 → **0**、`ai_etl_es_deadletter_depth` 90 → **0**、`ai_etl_es_deadletter_drained_total{outcome="drained"}=90` 且五个 outcome 序列在**启动时即存在**；那两份文档仍在索引里（按 `doc_id` 计数 31 + 1 = 32），短语召回仍能命中被 ack 的块（`doc-1788317207289560094_0007`） | `internal/es/queue.go` 的 `EnqueueDeadLetter` 全仓没有第二个引用点，`internal/es/deadletter.go` 新增 `DrainResolvedDeadLetters` 按**「意图是否已满足」**判定 —— 一条死信的意图很窄：它是因为某一块写不进 ES 才产生的，全部目的就是那次写入，所以 `chunk_id` 已在索引里 = 意图已满足 = 可 ack；查不到 = 缺口是真的、这条记录是它唯一的描述 = 留下。**判不了的一律留下**：presence 查询报错/超时/被拒、`op` 不是 `index`、JSON 读不懂、`chunk_id` 为空；presence 失败时**返回错误**而不是静默返回空（否则「查不动」会读成「什么都没有」）。判据是 `HTTPIndexer.PresentChunkIDs`，按 `_id` 判（chunk 写入时就用 `chunk_id` 当文档 id，是恒等判定，不是可能命中别的块的文本检索）。`RetryQueue` 增 `DeadLetterEntries` / `AckDeadLetterEntries`，**ack 按值不按下标**（下标在下次写入前才有效），两个实现都补 —— 漏补会编译失败而不是静默不 drain。周期由 `ES_DEADLETTER_DRAIN_INTERVAL`（默认 5m，0 关闭，三处齐）控制，首轮**不等第一个 tick**（要处理的正是进程启动时已在列表里的那批）。扫描是**先读完整页再 ack**：按 offset 分页只在列表不变时成立，延后 ack 同时把「每条一次写」变成「每轮一次写」；也因此头部一批判不了的记录不会挡住后面的。**不做重放**（理由同缺陷 28）。反向验证：把判定短路成无条件 ack（在 `present[...]` 那一支上直接取真）→ `TestDrainRemovesOnlyTheChunksTheIndexHolds` 与 `TestDrainPagesPastRecordsItRetains` 双红；复原后 `deadletter.go` 的 sha256 不变（`f359a02c…`） | `2a2fba9` |

### 1.3.1 跨层索引一致性：对账已建、扩到检索过滤字段与登记表计数、并把历史数据回填完（`499ed26` `aae0d59` `6930663` `67d79ab` `4c995c4` `5989882` `60756c1`），`citation_unverifiable` 17 → 5（12 份已修为缺陷 23 / 24 / 25，剩余 5 份已逐条归因为「按设计隐藏」）、`keyword_unsearchable` 2 → 0（真因是 ES 越 flood-stage 水位后拒写，chunk 耗尽重试落进死信列表；那份列表当时没有消费者，于是差异被记录下来却没人处理 —— 消费者已由缺陷 30 补上，`2a2fba9`），13 条已归因的先决条件进基线，脚本已接 CI
...
| `keyword_unsearchable` | **2 → 0** | 已发布文档在 ES 里一个块都没有 —— 全文召回静默丢掉这份文档 | **已回填**（`60756c1`，按存储重建 32 行，不删文档、不重解析、不换 doc_id）；真因是 ES 越 flood-stage 水位后拒写，重试耗尽落进死信列表，而那份列表当时没有消费者（见下；消费者已由缺陷 30 补上） |
...
所以 `chunk_id` 已在索引里 = 可 ack，查不到 = 缺口是真的、留下；**判不了的一律留下**，因为 ack 掉的是那份
分歧唯一的描述，而留下的代价只是几 KB 和面板上一行。磁盘那边，结论是**没有一次性解法**：清到 63GB 可用四天
就涨回 95%，回收之后做一次重建又吃回 5GB —— 所以交付的是一个**默认 dry-run、带范围守卫、可重复执行**的脚本，
并且如实报告「本项目自己的可回收残留只剩约 1GB，剩下约 34GB 是别人的」。这两件事的共同点是
**不能靠「看起来处理过了」交差**：队列要能说出每一条为什么留下，磁盘要能说出每一 GB 为什么没动。

---

## 8. 边界与未做的事（避免误解）

- **未完成事项的唯一出处是 [`docs/open-items.md`](open-items.md)，不是本文。** 2026-09-26 用户问
  「还有哪些没有做的工作」，我凭回忆列了一份，把**我该做而没做完**的东西（对账 CI 从没跑过、
  切块改动没跑评测轨）归进了「可做可不做」那一堆 —— 用户反问「你为什么会分类错呢」。
  根因是两条：① 我用「要不要用户参与」这个维度分堆，却给其中一堆起了带价值判断的名字（维度是
  「谁来做」，名字承诺的是「重不重要」）；② **清单是凭回忆列的，而回忆会漏** —— 本文里早就写着
  「未端到端验证」「没跑过」，机械扫描就能拿到，我却没去扫。
  所以现在这份清单**不是给人凭印象读的**：`scripts/extract-open-items.py` 扫出本文里每一处欠账词，
  强制 `open-items.md` 给每一处归宿（登记成带 `owner` 的条目，或明确判为「不是欠账」并给理由），
  `scripts/tests/test_open_items_contract.py` 把这件事钉住（含三种篡改的反向验证）。
  `owner` 只有四个取值：**`agent`（我该做、没做完）、`user`（要用户点头或定口径）、`external`
  （外部依赖）、`declined`（已决定不做，必须带理由）** —— 跑 `--check` 会打印四个计数，
  **`agent` 那个数就是被「可做可不做」掩盖掉的量**。
- **代码改动集中在这些章节**：§1.3 列出的 30 个缺陷，§1.1.2 的 ES 水位百分比化 + 宿主机磁盘告警
...
- **ES 死信列表（`es:index:deadletter`）现在有界、可观测、也有消费者了（缺陷 28，`17d436f` 加界与观测；缺陷 30，`2a2fba9` 加消费者）**。
  ...
  ① 列表**有上限了**（`ES_DEADLETTER_MAX`，默认 2000，超出丢最旧的），因为 `maxmemory-policy=noeviction` 下无界 list 的症状是**先填满实例、再让所有写入一起失败**（含摄入本身），而线上实测每条约 24.6KB（`RetryMessage` 内嵌 chunk 与嵌入向量）、90 条已占 2.1MB；
  ② 深度与丢弃进了指标（`ai_etl_es_deadletter_depth` 在 worker 启动时**播种**，所以它在**第一次故障之前**就有序列），丢弃配了告警 `ESDeadLetterDropped`（**不设 `for`**）；
  ③ 消费者 `DrainResolvedDeadLetters` 按**「意图是否已满足」**判定 —— 一条死信的意图很窄：它是因为某一块写不进 ES 才产生的，全部目的就是那次写入，所以 `chunk_id` 已在索引里 = 可 ack，查不到 = 缺口是真的、这条记录是它唯一的描述、留下；**判不了的一律留下**（presence 报错/超时/被拒、`op` 不是 `index`、JSON 读不懂、`chunk_id` 为空），且 presence 失败时**返回错误**而不是静默返回空 —— 否则「查不动」会读成「什么都没有」。周期由 `ES_DEADLETTER_DRAIN_INTERVAL` 控制（默认 5m，0 关闭），首轮不等第一个 tick。2026-09-26 线上实测：`LLEN` 90 → **0**、`ai_etl_es_deadletter_drained_total{outcome="drained"}=90`，而那两份文档仍在索引里（31 + 1 = 32 块），短语召回仍能命中被 ack 的块。
  **不做重放**，理由写在 §1.3 第 28 行的处置列：`RetryMessage` 是整块 chunk 的替换式写入，重放等于用故障时刻的旧副本替换当前索引，而真缺口的正确修复是重跑摄入（对账已能发现并调度）。
  早先那一轮做的是把**已经丢掉的那 2 份**回填回去、并让对账能自动发现同类问题。
- **宿主机磁盘：2026-09-26 的复测、回收，以及为什么它没有「解决」（`scripts/reclaim-host-disk.sh`，`b131844`）**。
  实测起点 `197G / 179G 已用 / 10G 可用 / 95%`（记录里的「90%」已过期），水位实测 low 95% / high 96% / **flood_stage 97%**（不是默认的 95%），ES `green`、**没有 `read_only` 块** —— 即当时还没出事，但离只读只差 2 个点（约 4GB）。
  脚本默认 dry-run、`--apply` 才动；镜像只动本项目隔离栈残留、逐个用 `docker ps -a --filter ancestor` 守卫；**不碰卷、不碰基础镜像、不碰别的项目**；跑完打印前后 `df` 与 `docker system df`。执行后 92%（16GB 可用）。
  三处被实测推翻的想当然：① 构建缓存的旋钮是 `--min-free-space` 而不是 `--max-used-space` —— 缓存报 55.21GB，其中 51.29GB 是 `Shared`（与镜像共用的层）、私有只有 3.93GB，所以上限 20GB 回收 **0B**，换 `--min-free-space 40GB` 才回收 4.04GB；② 删掉的 tag 数不等于释放的字节数 —— 32 个 `ai-etl-*` 隔离栈镜像合计 8.8GB，`df` 只涨 2GB，因为它们与线上栈同源、层是同一批 blob；③ 99 个 `LINKS=0` 的卷共 19.80GB，只读探查最大的一组（9 个 1.23GB）内容是 `dingtalk/napcat/qqbot/wecom` 与一个 pnpm store —— **是别的项目的**，另有承重的 Go 构建缓存，所以「未引用」不等于「可丢」。
  **为什么没有解决**：回收之后做了一次 `docker compose up -d --build etl-worker`，磁盘回到 95%（11GB 可用）—— **一次重建约 5GB**。所以这条的正确形态是「可重复执行的回收」，不是「一次清空」（2026-09-23 清到 63GB 可用，四天内涨回 95%）。
  **剩下的约 34GB 未引用镜像不是本项目的**：`golang:*` 八个版本约 9.2GB、`node:22-bookworm` / `node:20-bookworm` / `python:3.12` / `mcr.microsoft.com/playwright` 约 6.7GB、`openclaw` 三个旧 tag 11.93GB、`keycloak` 708MB、`grafana:latest` 1.58GB、prom 旧版 1.45GB。本项目自己的未引用镜像只有约 1GB（`ai-etl-platform-alertmanager:latest` 122MB + 三个 `ghcr.io/xjsmx0908/ai-etl-platform-*:master` 906MB）。**动这 34GB 要用户点头**：`openclaw:custom-lobster-backup-20260721` 与 `ghcr.io/openclaw/openclaw:2026.7.1` 是别人刻意留下的回滚点，而删共享构建基镜像会让下一次构建变成网络拉取。
...
- **更正（2026-09-24 深夜，本轮自查）：本文此前写的「死信没有重放入口」是高估。** 入口是
  ...
  所以正确的说法是：**入口在、对这些行失灵**，而不是「没有入口」。（「没有消费者」这句到 2026-09-26 不再成立 —— 缺陷 30 补上了消费者，见上一条。）
