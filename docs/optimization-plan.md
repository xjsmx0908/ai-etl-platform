...
| 已知未修 | **跨层索引一致性的剩余项**（§1.3.1）：对账脚本 `scripts/check-index-consistency.py`（`499ed26` `aae0d59` `6930663` `67d79ab`）已把四层摆在一起比。其中「引用无法在页面上核对」**17 份已修掉 12 份**（缺陷 23 / 24 / 25，`99c119a` `4133512` `0e12588`），**剩余 5 份已逐条归因为「按设计隐藏」**；登记表块数那 **99 份已回填**（`5989882`，99 → 0）；仍挂着的还有 **2 份**已发布文档不在全文索引里、**45 份**存储点缺空间过滤键、**48 份**同一 chunk id 有重复存储点（都是历史数据回填，待决策） |
...
（上表是报告 `artifacts/index-consistency/index-consistency-20260924T073732Z.json` 的读数，即回填并修好分页之后的线上状态；`permission_drift` 特意留着，因为删掉它会让读者以为没检查过。上一版报告 `072325Z` 里 `count_mismatch` 是 1、`registry_count_stale` 是 99 —— 两处变化的原因都在下面两段。）
