...
| 已知未修 | **跨层索引一致性的剩余项**（§1.3.1）：对账脚本 `scripts/check-index-consistency.py`（`499ed26` `aae0d59`）已把四层摆在一起比。其中「引用无法在页面上核对」**17 份已修掉 12 份**（缺陷 23 / 24 / 25，`99c119a` `4133512` `0e12588`），**剩余 5 份已逐条归因为「按设计隐藏」**；仍挂着的还有 **2 份**已发布文档不在全文索引里、**1 份** ES/Qdrant 块数不一致、**45 份**存储点缺空间过滤键、**48 份**同一 chunk id 有重复存储点（后三项都是历史数据回填，待决策） |
...
### 1.3.1 跨层索引一致性：对账已建并扩到检索过滤字段（`499ed26` `aae0d59`），`citation_unverifiable` 17 → 5（12 份已修为缺陷 23 / 24 / 25，剩余 5 份已逐条归因为「按设计隐藏」），其余待决策
...
（上表是同一份报告 `artifacts/index-consistency/index-consistency-20260924T064706Z.json` 的读数；`permission_drift` 特意留着，因为删掉它会让读者以为没检查过。）
