...
1. 只有达到 10 分钟边界的联邦 `ps1_` 和已登记 high-risk action 可以启动；风险 action
   由服务端响应提供，前端不自行推断。
2. callback 复用已配置的 OIDC redirect URI；只有 callback state 匹配独立 reauth state
   Cookie 才进入重新认证分支，遗留 Cookie 不劫持普通登录。可信证据、当前 credential、
   tenant/subject、实时用户状态全部通过后才原子轮换。
...
2. `SESSION_MAX_ACTIVE_SESSIONS` 默认限制每个内部用户最多 3 个活跃会话（演示基线接受
   1～3）；超限建立在用户锁保护的同一事务中按持久化 `creation_order` 撤销最旧会话、
...
3. `GET /v1/auth/sessions` 与 `DELETE /v1/auth/sessions/{handle}` 只接受当前有效 `ps1_`。
   ...
   或不活跃句柄不泄露存在性且不写审计，成功撤销与 `session_device_revoked` 审计一起提交。
