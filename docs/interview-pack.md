...
5. **核心发现**：锚点集的 100% 是假象，真实语义检索 Recall@1 只有 19%
...
**关键教训：mock 的 100% 是假象。** 锚点数据集每条 query 内嵌 `alpha001` 这类 token，路由器判定为精确匹配，语义检索路径从未执行。换真实模型 + 语义数据集后，Recall@1 从 86% 掉到 **19%**——这才是系统真实能力。这个发现说明：**评测保真度决定一切后续优化的可信度**。
...
**结果不是「100%」**，而是：语义集 Recall@1 19%、@5 64%、负样本 6/6、pass_rate 69%。这些数字诚实反映了系统的边界，也指明了改进方向（中文 embedding 选型、query 改写、中文 reranker）。
...
### 为什么你的 Recall@1 只有 19%？

这是真实能力，不是失败。nomic-embed-text 是 137M 的小模型，对中文口语 query 理解有限；ES 默认分词对中文弱；语料同质（企业知识库文档主题相近）。**锚点集能到 86% 是因为它测的是关键词匹配。** 19% 才是系统真实水平，也指向明确改进方向——换中文 embedding、query 改写、中文 reranker。
...
| 维度 | 数字 | 说明 |
| --- | --- | --- |
| 语义集 Recall@1 / @5 | 19% / 64% | 真实模型，口语 query |
| 负样本拒答 | 6/6 | 修复后 |
