...
| UAT-026 | 使用逻辑 | S3 | `/login` | 全部 | 已修复 | 两个体验入口的文案不表达角色，低权限视图被误读成「系统就这么点功能」 | 你的首页是不是有问题啊？就是体验登录点入之后是普通用户只能看到的一些特，一些东西，体验发布预审反而能够看到完整的功能。 |
| UAT-027 | 使用逻辑 | S3 | `/qa` | 全部 | 已修复 | 演示租户只有 1 份可答文档，普通用户视角问什么都弃答 | 2026-09-25 复验：普通用户身份三问全部作答且 `refusal_reason=none`，来源命中新增文档 |
| UAT-028 | 功能缺陷 | S2 | `/qa` | 全部 | 已修复 | 指代上文的问句被系统自己挑一个指代对象作答：引文忠实、grounding 通过，错的是前提 | 只有「刚才那份文档里还写了什么？」这类，它会自己挑一个指代对象（引文忠实，但指代是猜的） |
...
### UAT-026 登录页两个体验入口的文案不表达角色

- 类型：使用逻辑 · 级别：S3 · 状态：已修复
- 页面：`/login` · 角色：全部
- 复现：
  1. 打开登录页，点「体验问答」→ 落到 `/qa`，只有普通用户的可见范围。
  2. 返回登录页，点「体验发布预审」→ 落到 `/release-center`，能看到完整的管理员功能。
- 期望：入口名称本身说明这是**角色选择**，访客不会把低权限视图误读成「系统就这么点功能」。
- 实际：两个按钮是**刻意的角色入口**（`onDemo(account)` → `account === "admin" ? "/release-center" : "/qa"`，`web/app/login/page.tsx:71-83`），权限差异是 RBAC 生效的证据，**不是权限缺陷**。但文案「体验问答」/「体验发布预审」读起来像「主入口」与「子功能」，而不是「普通用户」与「管理员」。叠加 `demo` 租户只有 1 份已发布文档，点「体验问答」进去几乎无内容可问（2026-09-25 实测：问「员工手册里试用期是多久？」4.8s 后弃答 `insufficient_support`），放大成「首页有问题」的观感。
- 证据：`web/app/login/page.tsx:71-83`；`scripts/tests/test_demo_login.py`；真实页面复验 `.workbuddy-ai/tmp/probe-login-after.json`
- 归属：前端
- 建议：改文案表达角色，**不动 `grid-cols-2` 布局**（它被 `test_demo_login.py:15` 钉住）。
- 修复：分隔线 `体验演示` → `体验演示 · 选择角色`；按钮可见文案 `体验问答` → `普通用户视角`、`体验发布预审` → `管理员视角`。`aria-label` **刻意保留**（`scripts/web-page-probe.cjs:234` 用它定位按钮，改了会打断 `--login` 复验路径），契约测试断言同步更新为新可见文案 + 原 `aria-label`。
- 复验：2026-09-25，容器重建 `web` 后，1440×900 走真实页面（探针 `--url /login`）：渲染出「体验演示 · 选择角色」「普通用户视角」「管理员视角」，且**不再出现**「体验问答」「体验发布预审」。契约测试 `test_demo_login.py` 3/3 通过、`npm run lint` 无警告。
- 未做：没有合并成单一入口（那要改会话逻辑）。**`demo` 租户内容过少**当时被列为独立问题，已于同日立为 UAT-027 并修复，见下条——两条是同一段观感的两个成因。

### UAT-027 演示租户只有 1 份可答文档，普通用户视角问什么都弃答

- 类型：使用逻辑 · 级别：S3 · 状态：已修复
- 页面：`/qa`（经 `/login` 的「普通用户视角」入口）· 角色：全部
- 复现：
  1. 打开登录页，点「普通用户视角」→ 落到 `/qa`。
  2. 问任意业务问题，例如「员工手册里试用期是多久？」。
- 期望：演示租户能答出几类常见业务问题，访客据此判断「这个系统能答什么」。
- 实际：`demo` 租户只有 1 份已发布文档（`demo-doc-handbook`），2026-09-25 实测该问题 4.8s 后弃答 `insufficient_support`。它与 UAT-026 是**同一段观感的两个成因**：入口文案把低权限视图说成「主入口」，进去之后又确实没内容可问；两条叠加，用户才得出「首页有问题」。
- 真因：种子 `demoShowcaseDocuments()` 只写 3 份文档，其中 2 份**刻意**是未发布状态（`demo-doc-onboarding` 待审批、`demo-doc-payroll` 机密待双审）。这两个状态本身就是发布中心的演示内容，不能改；于是「可答文档」被压到 1 份。
- 修复：新增 3 份**已发布**文档、各 3 块、带具体数字（便于提问命中）——`demo-doc-travel`（差旅与报销标准：住宿分级 600/450/350 元、报销 15 个工作日）、`demo-doc-security`（信息安全管理规范：密码不少于 12 位、每 90 天更换）、`demo-doc-leave`（员工假期管理规定：年假 5/10/15 天）。**发布中心的两个演示状态一字未动**，`demo-doc-handbook` 的已发布状态同样未动。
- 反向验证：`TestEnsureDemoShowcaseIndex_IndexesEveryShowcaseDocument` 原先硬编码 `len(docs) != 3`，加文档后直接报红——但那个断言**钉的是总数而不是内容**，扩表就会挡住合法改动。改成「三份发布中心文档必须都在」后，临时把 `docID: demoPublishedDocID` 改为 `demo-doc-handbook-removed`，测试报红 `showcase document demo-doc-handbook is missing from the seed`，确认守卫没有空转，随后复原。
- 线上断言：`demo` 租户 `published` 1 → **4**（`draft` 仍 2）；`index_manifests` 6 条全部 `active`、`qdrant_count` 与 `elasticsearch_count` 各 3。普通用户身份实测三问全部作答且 `refusal_reason=none`：「住宿费标准是多少？」→ 一线 600 / 省会 450 / 其他 350；「员工年假有多少天？」→ 5 / 10 / 15 天；「账号密码多久需要更换一次？」→ 90 天；三问的首位来源分别是 `demo-doc-travel` / `demo-doc-leave` / `demo-doc-security`。
- 权限：新增文档沿用种子既有权限（两份 `internal`、一份 `public`），未放宽任何可见性判据；普通用户能答出 `internal` 文档是发布链路正常，不是因为跳过权限。
- 证据：`services/etl-worker/cmd/api/demo_showcase.go`、`demo_showcase_test.go`；断言脚本 `.workbuddy-ai/tmp/assert_demo_answerable.py`
- 归属：后端（演示种子）
- 建议：已修复。

### UAT-028 指代上文的问句被系统自己挑一个指代对象作答

- 类型：功能缺陷 · 级别：S2 · 状态：已修复
- 页面：`/qa` · 角色：全部
- 复现：
  1. 以普通用户身份（`POST /v1/auth/demo-login {"account":"user"}`）打开 `/qa`。
  2. 先问一个正常问题，再问「刚才那份文档里还写了什么？」。
  3. 再问「上面说的第二点是什么意思？」。
- 期望：不猜。要么弃答并说明「不知道你指的是哪一份」，要么请调用方把文档名或编号写进问题。
- 实际（修复前）：两句都以 `refusal_reason=none` 作答 —— 「刚才那份」被答成「最后一份文档（员工手册）还包含以下内容…」（来源 `demo-doc-handbook`），「上面说的第二点」被答成「应指第一条文档中的『机密数据须经数据所有者授权，访问行为全程留痕』」（来源 `demo-doc-security`）。两句的**引文都能在来源里逐字找到**、grounding 通过，所以既有引用校验一条都不响。对照两句不指代的问法：「它的上限是多少？」模型自己列出候选并反问（住宿/事假/年假）、「那这个呢？」直接弃答 —— 说明模型并非总在猜，缺陷是**没有判据**而不是模型不配合。
- 真因：两处。① `POST /v1/query` 的请求体只有 `question`，全仓没有 `conversation_id` / 会话历史，所以「指哪一份」这个信息在**协议层**就不存在；而检索总会返回若干份候选 —— 「挑一份」是必然发生的默认行为，不是偶发幻觉。② 没有任何代码检查问句里是否含**指代上文的说法**：`internal/query/service.go` 的五条弃答分支全部挂在「检索结果不够」上，而这里检索结果很够。
- 归属：后端
- 建议：在**访问校验之后、检索之前**加一道问题形态判据；命中即按新的弃答原因返回，不检索、不生成。
- 修复：新增 `internal/retrieval/reference_policy.go` 的 `HasUnresolvedReference`，用 **47 条整短语**词表（「刚才那份」「上面说的」「之前提到的」「上一轮」等）判定；命中返回第 6 个弃答原因 `context_required`，`sources` / `retrieved_sources` / `citations` 三种全空（`bb7b270`）。词表刻意用整短语而非「指示词 + 指代名词」运行时拼装，因为**拒掉一个能答的问题比它要替代的猜测更糟**；逐条排除项写进代码注释与 ADR 0012（裸「刚才」/「之前」、「上述内容」、「合同第三条」都不判；`RetrievalOnly` 诊断调用豁免，否则容量/诊断探针会被自己的探针句拦下）。弃答句三处同源同步：后端 `refusalAnswers`、前端 `web/app/(app)/qa/page.tsx` 的 `REFUSAL_EXPLANATIONS`、评测轨 `scripts/run-evals.py` 的 `NEGATIVE_FALLBACK_MARKERS` —— 漏补第三处会把**正确的弃答记成幻觉作答**；并用跨语言契约测试 `scripts/tests/test_refusal_sentence_contract.py` 把「每条弃答句评测轨都认得」钉住（`c77e55d`）。
- 决策记录：`docs/adr/0012-single-turn-qa-boundary.md`（`d15ac00`）—— 本次**不引入多轮对话**，理由与重审条件写在该 ADR。判据是「做不到的能力不能静默装成做得到」，不是「别人有的功能必须有」。
- 反向验证：Go 侧把判据改成恒假 → 11 个正例子测试 + 接线测试全红，反例不受影响，复原后 sha256 不变；Python 侧从 `run-evals.py` 移除 `"不保留会话上下文",` → 契约测试报红并**指名**未覆盖的那条句子，复原后 sha256 不变。
- 线上断言（重建 `query-api` 后）：正例 5 句（含带 ASCII 空格的「刚才 那份文档里还写了什么？」）全部 `context_required` 且 `sources` 为空；反例 6 句行为不变（「住宿费标准是多少？」照常作答并命中 `demo-doc-travel`、「员工手册里还写了什么？」照常作答、「入职之前需要准备什么材料？」/「刚才上传的文档多久可以检索到？」/「报销流程有哪几步？」仍是 `insufficient_support`、「合同编号 CT-9999-0001 的违约金比例是多少？」仍是 `exact_evidence_missing`）。**11/11 as expected**。
- 已知边界：裸序数指代（「第二点是什么意思？」）**不判** —— 无法与「合同第三条规定了什么？」区分。误判会拒掉一个能答的问题，代价比漏判大，故有意留空。
- 证据：`artifacts/product-experience-acceptance/2026-09-25-uat028/`（`probe-followup-before.txt` 修复前四句原始记录与判读、`assert-after.txt` 修复后 11/11 断言）
