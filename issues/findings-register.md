...
| UAT-007 | 使用逻辑 | S2 | `/quality` | 全部 | 开放 | 检索质量页面存在意义不清，且报告未挂载 | 我不知道检索质量页面存在的意义是什么? |
...
2026-09-09 已完成 D1 复验。旧九条不再处于「待复验」。开放项为 UAT-007、UAT-013。S1（UAT-010）与 UAT-011/UAT-012 已于同日修复并复验。

续跑：不重跑 D0–D4，不重读 ES `_source`，不重验 UAT-010/PX-06/PX-09/UAT-011/UAT-012。下一步 UAT-007 → UAT-013。证据 `artifacts/product-experience-acceptance/2026-09-09-fix/`。
...
- 页面：`/quality` · 角色：全部
- 复现：三角色打开 `/quality`。
- 期望：说明这是离线评测回归，不是当次问答评分；报告缺失时仍能读懂用途。
- 实际：仅显示「检索质量数据未找到（docs/evals/reports/latest.json 未挂载）。请确认 web 容器已挂载评测报告。」未解释页面用途。对应 PX-09 失败。
- 证据：`PX-09-readonly.png`、`PX-09-user.png`、`PX-09-admin.png`。
- 归属：前端 / 运维挂载
- 建议：空状态补一句「离线评测回归，不是当次问答分数」；现网 web 镜像挂上 `docs/evals/reports/latest.json`。验收中途不改代码、不重启栈。
