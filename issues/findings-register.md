...
| UAT-014 | 功能缺陷 | S2 | `/documents/[id]` `/qa` | admin | 开放 | 受管「上传新版本」发布后问答仍引用旧切块 | 2026-09-09 覆盖补测：已发布文档同时保留 80 元与 120 元切块，问答仍答 80 |

2026-09-09 已完成 D1 复验。旧九条不再处于「待复验」。UAT-001～013 已关闭。覆盖补测新增开放项 UAT-014。
...
- 归属：后端发布/替换切块生命周期
- 建议：替换入库完成后清理旧切块，或按 generation/version 过滤检索；禁止已发布文档长期并列两版正文。

## 本轮未建单的缺口

- 机密双审已于 2026-09-09-fix 补测通过，不再作为缺口。
- 文档退役后问答、管理员新建用户、个人空间上传新版本已于 2026-09-09-gap 补测通过。
- 受管替换版本：旧发布在换版过程中仍可问（符合辅助闭环）；换版后同一文档同时保留旧/新切块，问答未切到新内容，已建 UAT-014。
