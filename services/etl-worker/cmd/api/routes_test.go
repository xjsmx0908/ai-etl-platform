...
// took the API down at startup: `/v1/users/invites/{inviteID}` was registered
// alongside the pre-existing `/v1/users/{userID}/external-identities`, and
// neither is more specific than the other. Go's ServeMux panics on that, in
// main(), before the server ever listens -- the container crash-looped with
...
// TestInviteRoutesStayOffTheUserSubtree pins where the invitation API lives.
//
// The path is not cosmetic: /v1/users/invites/{id} cannot coexist with
// /v1/users/{userID}/external-identities. If someone moves it back under the
// user subtree, this fails next to the explanation instead of at container
// startup.
func TestInviteRoutesStayOffTheUserSubtree(t *testing.T) {
	byMux := routePatternsFromMain(t)
	var patterns []string
	for _, group := range byMux {
		patterns = append(patterns, group...)
	}

	for _, want := range []string{"/v1/invites", "/v1/invites/{inviteID}", "/v1/auth/invites/{token}", "/v1/auth/invites/accept"} {
		found := false
		for _, pattern := range patterns {
			if pattern == want {
				found = true
	...
	for _, pattern := range patterns {
		if strings.HasPrefix(pattern, "/v1/users/invites") {
			t.Errorf("invitation route %q is back under the user subtree, where it conflicts with /v1/users/{userID}/...", pattern)
		}
	}
}
