...
func TestEngineKeepsPublishedReleaseVisibleWhileReplacementAwaitsApproval(t *testing.T) {
	...
	engine := newPolicyEvalEngine(embedServer.URL, embedServer.Client(), map[string]Retriever{
		SourceQdrant: staticPolicyEvalRetriever{name: SourceQdrant, candidates: []Candidate{
			{ChunkID: "same", DocID: "doc-1", DocumentVersionID: "job-old", GenerationID: "gen-published", Content: "approved policy", Rank: 1},
			{ChunkID: "same", DocID: "doc-1", DocumentVersionID: "job-new", GenerationID: "gen-replacement", Content: "unapproved replacement", Rank: 2},
		}},
	...
	resolver := &visibilityResolverStub{visible: map[string]bool{"gen-published": true}}
	...
	if len(got.Sources) != 1 || got.Sources[0].GenerationID != "gen-published" || got.Sources[0].Content != "approved policy" {
		t.Fatalf("sources = %+v, want prior published release only", got.Sources)
	}
	if resolver.calls != 1 {
