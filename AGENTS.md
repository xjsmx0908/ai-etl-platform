...
- `bash scripts/backup-stack.sh`：备份权威状态与投影到 `~/backups/ai-etl-platform/<UTC 时间戳>/`，写 `manifest.json`（含完整性门禁）。
- `bash scripts/restore-stack.sh`：在隔离项目 `ai-etl-restore` 上恢复并断言（8 项）。脚本拒绝任何名字不含 `restore` 的项目，绝不碰演示栈的卷。
