...
- 已删除无后续用途的历史实验报告、压测快照和重复文档；README 收成现行入口。CI gold、验收脚本和 P2.5 身份设计保留。
...
- P2.4-R5 已关闭：审批通知可投递到企业微信/钉钉，外部工作流可通过 webhook 收事件并用 callback 回写现有审批。
- P2.4-R3 已关闭：预审报告默认 7 天过期，过期后自动重审同一版本，已发布记录保留原证据。
- 583 页扫描 PDF 已由用户独立验证，OCR 全流程不再是当前工作项。
- Agent 功能主线仍是企业级 Agent 审计功能完善；不得从历史 OCR、生产准入或身份部署记录自动推导下一项。
- P2.4-R2 已完成：租户隔离审批组、成员启停、空间/权限/风险策略、双人审批、自审控制和策略持久化/API 已交付；审批存储不可用时 fail-closed。
- 2026-09-09 新增并行轨道 P-UAT-1：产品体验验收。章程在 `docs/product-experience-acceptance.md`，问题记入 `issues/findings-register.md`。先巡检记录，再按严重级别整改，不在巡检中途改代码。
...
- 产品体验问题的逐条状态只维护在 `issues/findings-register.md`；本文件不复制，也不写「哪些还待复验」这类快照。
...
每完成一项，必须同步更新 `docs/backlog.md`、`LEARNINGS.codex.md` 和本文件，并运行受影响测试后提交、推送远程。
