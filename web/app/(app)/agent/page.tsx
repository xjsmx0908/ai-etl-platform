...
import { permissionLabel, spaceLabel } from "@/lib/docDisplay";
import type { AgentRun, Document, KnowledgeSpace, ReleaseOverviewItem, ReleaseRequest, ReleaseRequestDetail, ReleaseReview } from "@/lib/types";
...
// Mirrors the Go-side reviewFindingCodeLabels. A finding code is an identifier,
// so an unmapped one must not be printed as-is: the panel reads Chinese even
// when the planner invents a code the platform does not know.
...
const UNKNOWN_FINDING_LABEL = "其他问题";
...
function findingLabel(code: string) {
  return FINDING_LABELS[code] || UNKNOWN_FINDING_LABEL;
}

// The severity is a wire enum like the rest of them, and it was the one field
// the panel printed raw -- "严重度：medium".
function severityLabel(severity: string) {
  return ({ low: "低", medium: "中", high: "高", critical: "严重" } as Record<string, string>)[severity] || severity;
}
...
export default function AgentPage() {
  ...
  const refreshReleaseCenter = useCallback(async (quiet = false) => {
    ...
    try {
      const [docs, reqs, states, spaceList] = await Promise.all([
        apiClient.listDocuments({ limit: 100 }), apiClient.listReleaseRequests(), apiClient.listReleaseOverview(),
        apiClient.listKnowledgeSpaces().catch(() => ({ items: [] as KnowledgeSpace[] })),
      ]);
  ...
  const visible = useMemo(() => {
    ...
    return overview.filter((item) => overviewFilter === "all" || item.state === overviewFilter)
      .filter((item) => !query.trim() || `${item.file_name} ${item.document_id} ${item.knowledge_space_id} ${spaceLabel(item.knowledge_space_id, spaces)}`.toLowerCase().includes(query.trim().toLowerCase()))
  ...
  }, [overview, overviewFilter, overviewSort, query, spaces]);
