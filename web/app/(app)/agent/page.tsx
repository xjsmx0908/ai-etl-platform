      ...
      <div className="grid min-h-[620px] gap-4 lg:grid-cols-[360px_1fr]">
        ...
        <main className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          {!selectedOverview && !selectedRequest ? <div className="flex min-h-[560px] items-center justify-center text-sm text-slate-400">暂无业务记录</div> : <div className="space-y-5">
            ...
            {detail && <section className="rounded-lg border border-indigo-200 bg-indigo-50/40 p-4"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">Agent 预审</h3><Badge tone={detail.review.status === 'failed' || detail.review.status === 'error' ? 'danger' : detail.request.state === 'published' ? 'success' : detail.request.state === 'rejected' ? 'danger' : 'warning'}>{detail.review.status === 'failed' || detail.review.status === 'error' ? '预审失败' : REQUEST_LABELS[detail.request.state] || detail.request.state}</Badge></div><div className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><span className="text-xs text-slate-500">预审状态</span><p className="font-medium">{detail.review.status === 'completed' || detail.review.status === 'success' ? '已完成' : detail.review.status || '未知'}</p></div><div><span className="text-xs text-slate-500">风险评估</span><p className="font-medium">{riskLabel(detail.review.risk_level)}</p></div><div><span className="text-xs text-slate-500">Agent 建议</span><p className="font-medium">{recommendationLabel(detail.review.recommendation)}</p></div><div><span className="text-xs text-slate-500">管理员审批</span><p className="font-medium">{approvedCount} / {detail.request.required_approvals}</p></div></div><div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-500"><span>预审时间：{detail.review.created_at ? new Date(detail.review.created_at).toLocaleString('zh-CN') : '—'}</span><span>Prompt 版本：{detail.review.prompt_version || '—'}</span></div><p className="mt-3 text-xs text-slate-600">{detail.review.summary || 'Agent 未提供补充说明。'}</p>{detail.review.findings?.length ? <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3"><h4 className="text-xs font-semibold text-amber-900">预审发现</h4><div className="mt-2 space-y-2">{detail.review.findings.map((finding, index) => <div key={`${finding.code}-${finding.evidence_ref || index}`} className="rounded-md border border-amber-200/80 bg-white/70 p-2 text-xs"><div className="flex flex-wrap items-center gap-2"><Badge tone="warning">{findingLabel(finding.code)}</Badge><span className="font-medium text-amber-950">{finding.summary}</span><span className="text-amber-700">严重度：{finding.severity}</span></div>{finding.evidence_ref && <p className="mt-1 font-mono text-[11px] text-slate-500">证据块：{finding.evidence_ref}</p>}</div>)}</div></div> : <p className="mt-4 text-xs text-emerald-700">未发现需要人工关注的内容风险。</p>}{selectedDocument?.permission === 'confidential' && detail.review.risk_level === 'high' && detail.review.findings?.some((finding) => finding.code === 'sensitive_data_detected') && <p className="mt-3 text-xs text-amber-700">检测到敏感内容，风险升高，仍需双人审批。</p>}<p className="mt-2 text-xs text-slate-500">Agent 建议仅作为审核证据，不能自行批准或发布。</p>{detail.request.state === 'manual_exception' && <p className="mt-2 text-xs text-amber-700">Agent 预审不可用，管理员必须填写人工例外理由。</p>}{(detail.request.state === 'approval_pending' || detail.request.state === 'manual_exception') && !currentDecision && <div className="mt-4 flex flex-wrap gap-2"><input value={reason} onChange={(e) => setReason(e.target.value)} placeholder={detail.request.state === 'manual_exception' ? '人工例外理由（必填）' : '审批意见（可选）'} className="min-w-[220px] flex-1 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" /><Button size="sm" icon={Check} loading={busy} disabled={detail.request.state === 'manual_exception' && !reason.trim()} onClick={() => void decide('approved')}>批准发布</Button><Button size="sm" icon={X} variant="danger" disabled={busy} onClick={() => void decide('rejected')}>拒绝</Button></div>}{currentDecision && detail.request.state === 'approval_pending' && <p className="mt-3 text-sm text-indigo-700">你已提交决定，正在等待其他管理员处理。</p>}{currentDecision && detail.request.state !== 'approval_pending' && <p className="mt-3 text-sm text-emerald-700">审批已生效，发布流程已完成。</p>}<div className="mt-3 text-xs text-slate-500">审批记录：{detail.decisions.length ? detail.decisions.map((item) => `${item.decided_by} ${item.decision}`).join(' · ') : '暂无'}</div></section>}
