...
export function Logo({ size = "md", showWordmark = true, inverse = false }: { size?: "sm" | "md" | "lg"; showWordmark?: boolean; inverse?: boolean }) {
  ...
  return (
    <Link href="/" className="flex items-center gap-2.5">
      ...
      {showWordmark && <span className={`font-semibold ${inverse ? "text-white" : "text-slate-900"} ${text[size]}`}>知境</span>}
    </Link>
  );
}
